...
export function useDestinationVoting() {
  ...
  useEffect(() => {
    async function fetchVotingPageData() {
      try {
        const data = {
          trip_id: "001",
        ...
        setScores(Object.fromEntries(data.destinations.map((dest) => [dest.destID, 0])));
      } catch (error) {
  ...
  function handleScoreChange(destID: number, newScore: number) {
  ...
  return { tripDetails, destinations, scores, handleScoreChange, loading };
}
